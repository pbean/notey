...
- **2026-06-16 — DW-93 workspace-alignment renegotiation (human-approved, Pinkyd).** The frozen Boundaries→Always bullet *"Align workspaces by construction … spawn the CLI with `cwd: process.cwd()` … so the note passes the desktop's active-workspace filter"* (and the matching Design Notes "Workspace alignment is the crux") is **superseded** for hardening. The cwd-alignment held in CI but rested on two un-asserted assumptions (cwd-inheritance through the WebDriver launch; no stale persisted `activeWorkspaceId`) — logged as DW-93. Renegotiated approach: `cliLiveSyncTests` now drives the desktop into **All Workspaces** mode at suite start (StatusBar selector → `data-testid="workspace-all"`), making the note-list filter `null` (`listNotes(null)`) so the CLI note surfaces via the `note-created` seam regardless of workspace. The cross-process seam under test is unchanged (the refresh respects the All-Workspaces filter). Implemented in `spec-dw-91-93-e2e-hardening.md`; resolves DW-93. The frozen block above is retained as the original record — only this log entry governs the current behavior.
