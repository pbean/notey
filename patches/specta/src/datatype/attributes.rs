...
impl fmt::Debug for Attributes {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        for (key, value) in entries {
            // Use a wrapper struct instead of fmt::from_fn (which requires nightly debug_closure_helpers)
            struct FmtDynWrapper<'a>(&'a dyn DynAttributeValue);
            impl<'a> fmt::Debug for FmtDynWrapper<'a> {
                fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
                    self.0.fmt_dyn(f)
                }
            }
            map.entry(key, &FmtDynWrapper(value.as_ref()));
        }
        map.finish()
    }
}
