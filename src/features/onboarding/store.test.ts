 ...
 */
describe.skip('useOnboardingStore (red-phase: Stories 8.1, 8.3)', () => {
  beforeEach(() => {
  ...
  it('keeps the overlay hidden when onboarding is already complete (8.1)', async () => {
    vi.spyOn(api, 'loadOnboardingState').mockResolvedValue({ complete: true, sessionsSeen: 3 });
  ...
  it('shows the command hint only within the first 5 sessions (8.1)', () => {
    useOnboardingStore.setState({ sessionsSeen: COMMAND_HINT_SESSION_LIMIT - 1 });
