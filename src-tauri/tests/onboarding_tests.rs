...
//!
//! These tests lock the persisted onboarding contract now that the Story 8.1
...
fn mark_complete_is_idempotent() {
    ...
    assert_eq!(
        ...
        "idempotent completion must not reset sessions"
    );

    let contents = std::fs::read_to_string(onboarding::state_file_path(tmp.path())).unwrap();
    assert!(contents.contains("sessions_seen = 1"));
    assert!(!contents.contains("sessionsSeen"));
}
...
fn command_hint_retires_after_session_limit() {
    let within = OnboardingState {
        complete: true,
        sessions_seen: 0,
    };
    let last = OnboardingState {
        complete: true,
        sessions_seen: COMMAND_HINT_SESSION_LIMIT - 1,
    };
    let past = OnboardingState {
        complete: true,
        sessions_seen: COMMAND_HINT_SESSION_LIMIT,
    };
